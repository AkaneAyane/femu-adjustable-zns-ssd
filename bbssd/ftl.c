...
static void ssd_init_lines(struct ssd *ssd)
{
    ...
    lm->tt_lines = spp->blks_per_pl;//当前实现下，plane和lun是等价的，1个lun对应一个plane
...
static void ssd_advance_write_pointer(struct ssd *ssd)
{
    ...
    if (wpp->ch == spp->nchs) { //这里是优先使用通道，其次再考虑lun
        ...
        wpp->lun++;
        /* in this case, we should go to next lun *///然后再考虑lun
...
static void ssd_init_params(struct ssdparams *spp)
{
    ...
    spp->blks_per_line = spp->tt_luns; /* TODO: to fix under multiplanes 在这只支持单plane*/
    spp->pgs_per_line = spp->blks_per_line * spp->pgs_per_blk;
    spp->secs_per_line = spp->pgs_per_line * spp->secs_per_pg;
    spp->tt_lines = spp->blks_per_lun; /* TODO: to fix under multiplanes 在这只支持单plane*/
